...
window.onload = function () {
    ...
    for(let j = 0; j < period.length; j++) {
        period[j].innerHTML = `${day}.${month}.${year}`
    }
    ...
    if(myWidth > 980) {
        let parallax = skrollr.init({forceHeight: false});
    }
    ...
    $('.advantages__content-switch figure').on('click', function () {
        ...
        if($(this).hasClass('active')) {
            ...
            $('.advantages__content-img img.' + activeColor).addClass('active');
            // for(let j = 0; j < advFigure.length; j++) {
    ...
    let catalogTitle = document.querySelector('.catalog__block.sneakers h3'),
    ...
    switchColor.on('click', function () {
        ...
        if($(this).hasClass('active')) {
            ...
            let change = () => {
                ...
                catalogBigImage.src = `img/catalog/${activeColor}.png`;

                for(let j = 0; j < catalogLink.length; j++) {
                    catalogLink[j].href = `img/catalog/${activeColor}/${j + 1}.jpg`;
                    catalogSmallImg[j].src = `img/catalog/${activeColor}/${j + 1}.jpg`;
                }
            };
    ...
    */
    ...
    if(/iPhone|iPod|iPad|Android/i.test(navigator.userAgent)){
        $('.banner__content a').attr('href', '#size');
